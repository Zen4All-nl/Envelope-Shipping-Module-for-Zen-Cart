 ...
 * @version $Id: envelop.php 4184 2006-08-21 03:36:36Z ajeh $
 */
/**
...
class envelop extends base {
  ...
  function envelop() {
    ...
    $this->sort_order = MODULE_SHIPPING_ENVELOP_SORT_ORDER;
    //$this->icon = '';
    $this->icon = DIR_WS_MODULES . 'shipping/icons/tnt_logo.png';
    ...
    if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_ENVELOP_ZONE > 0) ) {
      $check_flag = false;
      $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_ENVELOP_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
      while (!$check->EOF) {
  ...
  function quote($method = '') {
    ...
    $envelop_cost = split("[:,]" , MODULE_SHIPPING_ENVELOP_COST);
